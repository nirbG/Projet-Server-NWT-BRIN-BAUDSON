...
let HerosService = class HerosService {
    ...
    findSome(start, nb) {
        return this._herosDao.findSome(start, nb).pipe(operators_1.map(_ => (!!_ && !!_.length) ? _.map(__ => new heros_entity_1.HerosEntity(__)) : undefined));
    }
    findOne(id) {
    ...
    create(body) {
        return this._addHeros(body).pipe(operators_1.flatMap(_ => this._herosDao.create(_)), operators_1.catchError(e => e.code = 11000 ? rxjs_1.throwError(new common_1.ConflictException(`Hero with name '${body.name}' already exists`)) :
    ...
    delete(_id) {
        return this._herosDao.findByIdAndRemove(_id).pipe(operators_1.catchError(e => rxjs_1.throwError(new common_1.NotFoundException(e.message))), operators_1.flatMap(_ => !!_ ? rxjs_1.of(undefined) :
            rxjs_1.throwError(new common_1.NotFoundException(`Hero with id '${_id}' not found`))));
    }
    _addHeros(body) {
