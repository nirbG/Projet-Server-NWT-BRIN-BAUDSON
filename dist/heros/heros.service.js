...
let HerosService = class HerosService {
    ...
    findAll() {
        return rxjs_1.of(this._heros).pipe(operators_1.map(_ => (!!_ && !!_.length) ? _ : undefined));
    }
    findSome(start, end) {
    ...
    create(body) {
        return rxjs_1.from(this._heros).pipe(operators_1.find(_ => _.id === body.id), operators_1.flatMap(_ => !!_ ?
            rxjs_1.throwError(new common_1.ConflictException(`People with isbn '${body.id}' already exists`))
            : this._addComics(body)));
    }
    update(id, body) {
        return this._findHeroIndexOfList(id).pipe(operators_1.tap(_ => Object.assign(this._heros[_], body)), operators_1.map(_ => this._heros[_]));
    }
    delete(id) {
    ...
    _addComics(body) {
        return rxjs_1.of(body).pipe(operators_1.map(_ => Object.assign(_, {
            ennemi: [],
            allie: [],
        })), operators_1.tap(_ => this._heros = this._heros.concat(_)));
    }
